...
class PointSample(object):
    ...
    def _points_random_sampling(self,
        ...
        # add
        if num_samples < points.shape[0]:   # add 0915
        ...
        # add 0915
            ...
            if num_samples > points.shape[0]:
                extra_choices=np.random.choice(choices,num_samples - points.shape[0],replace=True)
                choices=np.concatenate((choices,extra_choices),axis=0)
            np.random.shuffle(choices)
